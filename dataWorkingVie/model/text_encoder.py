...
class VariancePredictor(torch.nn.Module):
    ...
    def _forward(self, xs, x_masks=None, is_inference=False):
        ...
        if x_masks is not None:
            x_masks = x_masks.transpose(1,-1).to(torch.bool)
...
class TextEncoder(nn.Module):
    ...
    def __init__(self, n_vocab, n_feats, n_channels, filter_channels, filter_channels_dp, n_heads, n_layers, kernel_size, p_dropout, use_softmax, use_decay, preprocess_config_path='/home2/tuannd/tuanlha/EXpressiveTTS/dataWorking/preprocess.yaml', model_config_path = '/home2/tuannd/tuanlha/EXpressiveTTS/dataWorking/model.yaml', window_size=None, spk_emb_dim=64, n_spks=1):
        ...
        self.duration_predictor  = DurationPredictor(in_channels=n_channels + (spk_emb_dim if n_spks > 1 else 0), filter_channels=filter_channels_dp, 
        ...
        print(f'pitch_quantization: {pitch_quantization}')
        print(f'energy_quantization: {energy_quantization}')
    ...
    def forward(self, x, x_lengths, sty, spk=None,
        ...
        if self.pitch_feature_level == "phoneme_level":
            ...
            # print(f'pitch_embedding.shape: {pitch_embedding.shape}')
        ...
        if self.energy_feature_level == "phoneme_level":
            energy_prediction, energy_embedding = self.get_energy_embedding(
                x, energy_target, x_mask, e_control
            )
        ...
        log_duration_prediction = self.duration_predictor(x_dp, x_mask) # (B, T, 1)
