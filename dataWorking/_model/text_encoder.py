...
class LengthRegulator(nn.Module):
    ...
    def LR(self, x, duration, max_len):
        ...
        for batch, expand_target in zip(x, duration):
            # TODO: quên chưa pad expand_target?
            try:
                expanded = self.expand(batch, expand_target)
                output.append(expanded)
                mel_len.append(expanded.shape[0])
            except:
                print(f'duration shape: {duration.shape}, x shape: {x.shape}')
                break
...
class TextEncoder(nn.Module):
    ...
    def __init__(self, n_vocab, n_feats, n_channels, filter_channels, filter_channels_dp, n_heads, n_layers, kernel_size, p_dropout, use_softmax, use_decay, preprocess_config_path='/home/tuannd/tuanlha/DEX-TTS-Project/preprocess.yaml', model_config_path = '/home/tuannd/tuanlha/DEX-TTS-Project/model.yaml', window_size=None, spk_emb_dim=64, n_spks=1):
        ...
        self.arousal_embedding = nn.Embedding(
            n_bins, self.model_config["transformer"]["encoder_hidden"]
        )
        self.valence_embedding = nn.Embedding(
            n_bins, self.model_config["transformer"]["encoder_hidden"]
        )
    ...
    def forward(self, x, x_lengths, sty, spk=None,
        ...
        # FIXME: co the khong dam  bao du chieu du lieu o day
        ...
        if duration_target is not None:
            print(f'Train with duration target')
            x, mel_len = self.length_regulator(x, duration_target, max_len)
            duration_rounded = duration_target
        else:
            print(f'Train without duration target')
