...
def split_train_val_test(write_path, wav_path, mel_path):
    ...
    for mel_file in mel_filelist:
        ...
        basename = mel_file[:-8]
        text_path     = os.path.join(wav_path, basename+'.lab')
        
        with open(text_path, "r") as f:
            txt = f.readline().strip("\n")
        file_path = os.path.join(mel_path, mel_file)
        strings   = '|'.join([file_path, txt, '' + '\n'])
    ...
    with open("test_sentence/lmh_final_sentence.txt", "w", encoding="utf-8") as file:
...
def main(cfg):
    ...
    ### Save filelist ####
    split_train_val_test(write_path, wav_path, mel_path)
    
    
    ## filter for unseen spekaer ####
    make_unseen_filelist(write_path)



if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", type=str, default="config/LMH_final/preprocess.yaml")
