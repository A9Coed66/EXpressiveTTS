...
def split_train_val_test(write_path, wav_path, target_path, spk_dict):
    ...
    filelist  = []
    text_list = []
    for target_file in target_filelist:
        
        spk, basename = target_file.split('-')[0], target_file.split('-')[-1][:-5]  # remove .json
        ...
        file_path = os.path.join(target_path, target_file)
...
def main(cfg):
    ...
    target_path   = f'{cfg.path.preprocessed_path}/target_normalized'
    MakeDir(write_path)
    ...
    ### Save filelist ####
    split_train_val_test(write_path, wav_path, target_path, spk_dict)
