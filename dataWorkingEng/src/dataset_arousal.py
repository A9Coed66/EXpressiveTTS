...
class TextMelSpeakerDataset(Dataset):
    ...
    def get_data(self, line):
        ...
        # filepath, text, speaker, arousal, dominance = line[0], line[1], line[2], line[3], line[4]
        ...
        # return (text, mel, ref, sty, lf0, speaker, arousal, dominance)
    ...
    def __getitem__(self, index):
        ...
        # text, mel, ref, sty, lf0, speaker, arousal, dominance = self.get_data(self.filelist[index])        
        ...
        # item = {'y': mel, 'x': text, 'ref':ref, 'sty':sty, 'lf0':lf0, 'spk': speaker, 'arousal':arousal, 'dominance':dominance}
...
class TextMelSpeakerBatchCollate(object):
    def __call__(self, batch):
        ...
        spk = []
        # arousal   = torch.zeros((B, 1), dtype=torch.float32)
        # dominance = torch.zeros((B, 1), dtype=torch.float32)
        y_lengths, x_lengths, ref_lengths, sty_lengths, lf0_lengths = [], [], [], [], []

        for i, item in enumerate(batch):
            # y_, x_, spk_, ref_, sty_, lf0_, arousal_, dominance_ = item['y'], item['x'], item['spk'], item['ref'], item['sty'], item['lf0'], item['arousal'], item['dominance']
        ...
        # return {'x': x, 'x_lengths': x_lengths, 'y': y, 'y_lengths': y_lengths, 'spk': spk, 'ref':ref, 'ref_lengths':ref_lengths, 'sty':sty, 'sty_lengths':sty_lengths, 'lf0':lf0, 'lf0_lengths':lf0_lengths, 'arousal':arousal, 'dominance':dominance}
