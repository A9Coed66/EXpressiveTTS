...
def test(cfg, sample_size):
    ...
    ckpt  = torch.load(f'{cfg.checkpoint}/model-val-best.pth', map_location=cfg.device)
    if cfg.test.ema:
    ...
    for i, text in enumerate(tqdm(texts)):
        ...
        with torch.no_grad():
            y_enc, y_dec, attn = model(x, x_lengths, ref, ref_lengths, sty, sty_lengths, lf0, lf0_lengths, n_timesteps=cfg.n_timesteps, spk=spk, temperature=1.5)
