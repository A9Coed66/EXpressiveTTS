...
from model_ import monotonic_align
from model_.ref_encoder import *
from model_.text_encoder import TextEncoder
from model_.diffusion import Diffusion
from model_.utils import sequence_mask, generate_path, duration_loss, fix_len_compatibility
...
class DeXTTS(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, x, x_lengths, ref, ref_lengths, sty, sty_lengths, lf0, lf0_lengths, n_timesteps,arousal, dominance,  
        ...
        #LOG
        print(f'Arousal shape: {arousal.shape}, dominance shape: {dominance.shape}')
        assert sty_dec.shape == emotion.shape, f"sty_dec.shape: {sty_dec.shape}, emotion.shape: {emotion.shape}"
